...
import { MessageSquare, Users, Plus, X, Check, Bell } from "lucide-react";
...
export default function Sidebar() {
    ...
    const pendingRequests = useQuery(api.requests.listPendingRequests);
    const pendingCount = pendingRequests?.length ?? 0;

    return (
        <div className="flex flex-col h-full w-full bg-[hsl(var(--card))] border-r border-[hsl(var(--border))] relative overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-[hsl(var(--border))]">
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <button
                        ...
                        className="relative w-8 h-8 rounded-lg flex items-center justify-center text-[hsl(var(--muted-foreground))] hover:text-white hover:bg-white/10 transition-colors"
                    >
            ...
            {/* Requests panel (slides over sidebar content) */}
            {showRequests && (
...
function GroupChatDialog({ onClose }: { onClose: () => void }) {
    ...
    const sendGroupInvites = useMutation(api.requests.sendGroupInvites);
    ...
    const handleSendInvites = async () => {
        ...
        setLoading(true);
        try {
            await sendGroupInvites({ memberIds: selectedIds, groupName: groupName.trim() });
            setSent(true);
            ...
            <div className="w-full max-w-sm bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
                ...
                {sent ? (
                    <div className="p-8 text-center">
                        ...
                        <p className="text-sm font-medium text-white">Invites sent!</p>
                        <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">
                            The group will be created once a member accepts.
                        </p>
                    </div>
                ) : (
                    <>
                        <div className="p-4 space-y-3">
                            ...
                            {selectedIds.length > 0 && (
                                <p className="text-xs text-[hsl(var(--muted-foreground))]">
                                    Invites will be sent to {selectedIds.length} user{selectedIds.length !== 1 ? "s" : ""}
                                </p>
                            )}
                        </div>
